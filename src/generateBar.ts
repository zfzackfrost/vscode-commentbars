...
async function insertCommentBar(lines: string[], editor: Editor, selection: Selection): Promise<any> {
...
export async function commentBarGenerateCommand(advanced: boolean): Promise<any> {
	let editor = Window.activeTextEditor;
	if (!editor) {
		Window.showErrorMessage("There must be an active file to use this command!");
		return;
	}
	...
	let commentDelimsOrError: Delims.LanguageCommentDelims | string = Delims.processCommentDelimsConfig(config, languageId);
	...
	if (edtSelection.isEmpty) {
		...
		if (edtSelection.isSingleLine) {
			...
			Window.showErrorMessage("Multi-line comment bar text is not supported!");
			return;
		}
	}
	...
	if (advanced) {
		...
		if (_.isArray(presetsConfig) && presetsConfig.length > 0) {
			let items: QuickPickItem[] = _.map(presetsConfig, (preset: Presets.QuickPreset) => {
			...
			let styleChoice: QuickPickItem | undefined = await Window.showQuickPick(items, styleOpts);
			let presetIdx = _.findIndex(presetsConfig, (value: Presets.QuickPreset, index: number, collection: ArrayLike<Presets.QuickPreset>) => {
				if (styleChoice) {
	...
	if (commentDelimsOrError && fillChar && (text !== null)) {
		let lines: string[] = buildCommentBar(width, text, thickness, fillChar, commentDelimsOrError, seamlessFill);
		if (lines) {
